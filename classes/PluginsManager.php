...
class PluginsManager {
	...
	private function getImageList($ssId, $width){
		...
		foreach($imageArr as $igmAr => $imgIdArr){
			...
			$hideCaptionClick = "$('#slidecaption".$imgIdArr["imgid"]."').hide();$('#showcaption".$imgIdArr["imgid"]."').hide();";
			$html .= '<div id="slidecaption'.$imgIdArr["imgid"].'">';
			$html .= '<a href="#" style="font-size:9px;text-decoration:none;float:right;clear:both;margin-right:5px;" onclick="'.$hideCaptionClick.'">HIDE CAPTION</a>';
			...
			$html .= '</b></div>';
			$html .= '</div>';
			$showCaptionClick = "$('#slidecaption".$imgIdArr["imgid"]."').show();$('#showcaption".$imgIdArr["imgid"]."').show();";
			$html .= '<a href="#" id="showcaption'.$imgIdArr["imgid"].'" style="font-size:9px;text-decoration:none;float:right;clear:both;margin-right:5px;display:none;" onclick="'.$showCaptionClick.'">SHOW CAPTION</a>';
			$html .= '</div></div>';
		}
		$html .= '</div></div></div>';
	...
	public function createQuickSearch($buttonText, $searchText){
		$clientRoot = $GLOBALS['CLIENT_ROOT'];
		if(!$searchText) $searchText = 'Search Taxon';
		if(!$buttonText) $buttonText= 'Search';
		$html = '<script type="text/javascript">
					...
					$( "#quicksearchtaxon" )
						...
						.autocomplete({
							source: function( request, response ) { 
								$.getJSON( "'.$clientRoot.'/collections/rpc/taxalist.php", { 
			...
			<form name="quicksearch" id="quicksearch" action="'.$clientRoot.'/taxa/index.php" method="get" onsubmit="return verifyQuickSearch(this);">
				<div id="quicksearchtext" >'.$searchText.'</div>
