...
class TaxonProfile {
	...
	public function getTaxon($t,$isFinal=0){
		$t = trim($t);
	...
	public function setAccepted(){
		...
		$sql = "SELECT t.Tid, ts.family, t.SciName, t.Author, t.RankId, ts.ParentTID, t.SecurityStatus ".
				...
				"WHERE (ts.taxauthid = ".$this->taxAuthId.") AND (ts.Tid = ".$this->submittedTid.") ORDER BY t.SciName";
		$result = $this->con->query($sql);
		while($row = $result->fetch_object()){
	...
	public function setSynonyms(){
		if($this->tid){
			...
			$sql = 'SELECT t.tid, t.SciName, t.Author '.
					...
					'WHERE (ts.TidAccepted = '.$this->tid.') AND (ts.taxauthid = '.$this->taxAuthId.') AND ts.SortSequence < 90 '.
