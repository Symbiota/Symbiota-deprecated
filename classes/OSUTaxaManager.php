...
class OSUTaxaManager {
    ...
    private function setTaxaImages(){
        //$this->obsImageArr = array();
        //$this->specImageArr = array();
        ...
        $sql = 'SELECT t.sciname, ti.imgid, ti.url, ti.thumbnailurl, ti.originalurl, ti.caption, ti.occid, ti.tid, '.
            'IFNULL(ti.photographer,CONCAT_WS(" ",u.firstname,u.lastname)) AS photographer, c.CollType '.
            ...
            'INNER JOIN omoccurrences o ON ti.occid = o.occid '.
        ...
        while($row = $result->fetch_object()){
            ...
            if(($tId && !$occId) || (($tId && $occId) && (strpos(strtolower($collType),'observation') !== false))){
                $this->obsImageArr[$row->imgid]["url"] = $imgUrl;
                $this->obsImageArr[$row->imgid]["thumbnailurl"] = $tnUrl;
                $this->obsImageArr[$row->imgid]["photographer"] = $row->photographer;
                $this->obsImageArr[$row->imgid]["caption"] = $row->caption;
                $this->obsImageArr[$row->imgid]["occid"] = $row->occid;
                $this->obsImageArr[$row->imgid]["sciname"] = $row->sciname;
            }
            if(($tId && !$occId) || (($tId && $occId) && (strpos(strtolower($collType),'specimen') !== false))){
                $this->specImageArr[$row->imgid]["url"] = $imgUrl;
                $this->specImageArr[$row->imgid]["thumbnailurl"] = $tnUrl;
                $this->specImageArr[$row->imgid]["photographer"] = $row->photographer;
                $this->specImageArr[$row->imgid]["caption"] = $row->caption;
                $this->specImageArr[$row->imgid]["occid"] = $row->occid;
                $this->specImageArr[$row->imgid]["sciname"] = $row->sciname;
            }
        }
        $result->close();
    }

    public function echoImages($type, $start, $length = 0, $useThumbnail = 1){		//length=0 => means show all images
        $status = false;
        if(!isset($this->obsImageArr) || !isset($this->specImageArr)){
            $this->setTaxaImages();
        }
        if($type == 'obs'){
            $imageArr = $this->obsImageArr;
        }
        elseif($type == 'spec'){
            $imageArr = $this->specImageArr;
        }
        if(!$imageArr || count($imageArr) < $start) return false;
        $trueLength = ($length&&count($imageArr)>$length+$start?$length:count($imageArr)-$start);
        $spDisplay = $this->getDisplayName();
        $iArr = array_slice($imageArr,$start,$trueLength,true);
        foreach($imageArr as $imgId => $imgObj){
            //echo "<div id='sliderImgDiv'>";
            $imgUrl = $imgObj["url"];
            ...
            echo '<a href="'.$imgAnchor.'" target="_blank">';
