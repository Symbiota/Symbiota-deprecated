...
class KeyDataManager {
	...
	function getClFilterList($userid){
		...
		if($userid){
			...
			$result->free();
			if($returnList) $returnList["break"] = "------------------------";
		}

		//Get public checklist names
	...
	function setRelevanceValue($rel){
		$this->relevanceValue = ($rel?$rel:0);
	}
	
	function getRelevanceValue(){
