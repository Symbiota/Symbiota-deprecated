...
ALTER TABLE `actionrequest`
  ADD CONSTRAINT `FK_actionrequest_uid1`
   FOREIGN KEY (`uid_requestor` )
   REFERENCES `users` (`uid` )  ON DELETE SET NULL  ON UPDATE CASCADE, 
  ADD CONSTRAINT `FK_actionrequest_uid2`
   FOREIGN KEY (`uid_fullfillor` )
   REFERENCES `users` (`uid` )  ON DELETE SET NULL  ON UPDATE CASCADE, 
  ADD CONSTRAINT `FK_actionrequest_type`
   FOREIGN KEY (`requesttype` )
   REFERENCES `actionrequesttype` (`requesttype` )  ON DELETE NO ACTION  ON UPDATE CASCADE,
  ADD INDEX `FK_actionrequest_uid1_idx` (`uid_requestor` ASC),
  ADD INDEX `FK_actionrequest_uid2_idx` (`uid_fullfillor` ASC) ,
  ADD INDEX `FK_actionrequest_type_idx` (`requesttype` ASC) ;
