	...
	if($taxonRank > 180){
		...
		if($mapSrc){
			...
			if($taxonManager->getSecurityStatus() == 0 || $isAdmin || array_key_exists("CollAdmin",$userRights) || array_key_exists("RareSppAdmin",$userRights) || array_key_exists("RareSppReadAll",$userRights)){
				$gUrl = "javascript:var popupReference=window.open('".$clientRoot."/collections/googlemap.php?usecookies=false&type=3&db=all&thes=on&taxa=".$taxonManager->getSciName()."','gmap','toolbar=0,resizable=1,width=950,height=700,left=20,top=20');";
			}
					...
					foreach($sppArr as $sciNameKey => $subArr){
						...
						if(array_key_exists("map",$subArr) && $mapUrl = $subArr["map"]){
							...
							if($taxonManager->getSecurityStatus() == 1 || $isAdmin){
								$gUrl = "javascript:var popupReference=window.open('".$clientRoot."/collections/googlemap.php?usecookies=false&type=3&db=all&thes=on&taxa=".$subArr["tid"]."','gmap','toolbar=0,resizable=1,width=950,height=700,left=20,top=20');";
							}
							$aUrl = "";
