...
class OmOccurDeterminations {
 ...
 public function checkSchema() {
        ...
        if ($statement = $connection->prepare($preparesql)) {
            ...
            $statement->fetch();
            // compatible with 0.9.1.13.
            if ($versionnumber=='0.9.1.13') {
               $result = true;
            }
       }
       return $result;
...
*/
   ...
   private $loaded;
   private $error;
   const FIELDLIST = ' detid, occid, identifiedBy, dateIdentified, sciname, scientificNameAuthorship, identificationQualifier, identificationReferences, identificationRemarks, sortsequence, initialtimestamp, idbyid, isCurrent, appliedStatus, tidInterpreted,';
   const PKFIELDLIST = ' detid, ';
   const NUMBER_OF_PRIMARY_KEYS = 1;
   private $primaryKeyArray = array( 1 => 'detid'  ) ;
   private $allFieldsArray = array( 0 => 'detid' , 1 => 'occid' , 2 => 'identifiedBy' , 3 => 'dateIdentified' , 4 => 'sciname' , 5 => 'scientificNameAuthorship' , 6 => 'identificationQualifier' , 7 => 'identificationReferences' , 8 => 'identificationRemarks' , 9 => 'sortsequence' , 10 => 'initialtimestamp' , 11 => 'idbyid' , 12 => 'isCurrent' , 13 => 'appliedStatus' , 14 => 'tidInterpreted'  ) ;
...
/*tidInterpreted*/
  ...
  const C_tidInterpretedMYSQLI_TYPE = 'i';
   ...
   public function load($pk) {
        ...
        if($this->detid != NULL) {
           $sql = 'SELECT detid, occid, identifiedBy, dateIdentified, sciname, scientificNameAuthorship, identificationQualifier, identificationReferences, identificationRemarks, sortsequence, initialtimestamp, idbyid, isCurrent, appliedStatus, tidInterpreted FROM omoccurdeterminations WHERE detid = '.$this->detid ;

           $preparesql = 'SELECT detid, occid, identifiedBy, dateIdentified, sciname, scientificNameAuthorship, identificationQualifier, identificationReferences, identificationRemarks, sortsequence, initialtimestamp, idbyid, isCurrent, appliedStatus, tidInterpreted FROM omoccurdeterminations WHERE detid = ? ';

           if ($statement = $connection->prepare($preparesql)) { 
              $statement->bind_param("i", $this->detid);
              $statement->execute();
              $statement->bind_result($this->detid, $this->occid, $this->identifiedBy, $this->dateIdentified, $this->sciname, $this->scientificNameAuthorship, $this->identificationQualifier, $this->identificationReferences, $this->identificationRemarks, $this->sortsequence, $this->initialtimestamp, $this->idbyid, $this->isCurrent, $this->appliedStatus, $this->tidInterpreted);
              $statement->fetch();
              $statement->close();
           }
   ...
   public function save() {
        ...
        if ($this->detid!= NULL) {
            ...
            $isInsert = true;
            $sql .= '( detid ,  occid ,  identifiedBy ,  dateIdentified ,  sciname ,  scientificNameAuthorship ,  identificationQualifier ,  identificationReferences ,  identificationRemarks ,  sortsequence ,  initialtimestamp, idbyid, isCurrent, appliedStatus, tidInterpreted ) VALUES (';
            $sql .=  "  ? ";
            $sql .=  " ,  ? ";
            $sql .=  " ,  ? ";
            $sql .=  " ,  ? ";
        ...
        if ($statement = $connection->prepare($sql)) { 
           if ($this->detid!= NULL ) {
              $statement->bind_param("iisssssssisiiiii", $this->detid , $this->occid , $this->identifiedBy , $this->dateIdentified , $this->sciname , $this->scientificNameAuthorship , $this->identificationQualifier , $this->identificationReferences , $this->identificationRemarks , $this->sortsequence , $this->initialtimestamp, $this->idbyid, $this->isCurrent, $this->appliedStatus, $this->tidInterpreted   , $this->detid );
           } else { 
              $statement->bind_param("iisssssssisiiii", $this->detid , $this->occid , $this->identifiedBy , $this->dateIdentified , $this->sciname , $this->scientificNameAuthorship , $this->identificationQualifier , $this->identificationReferences , $this->identificationRemarks , $this->sortsequence , $this->initialtimestamp, $this->idbyid, $this->isCurrent, $this->appliedStatus, $this->tidInterpreted   );
           } 
           $statement->execute();
