...
class SpecProcNlpParserLBCC extends SpecProcNlp{
	...
	public function parse($rawStr) {
		...
		if(strlen($rawStr) > 0 && !$this->isMostlyGarbage2($rawStr, 0.50)) {
			...
			if($politicalConfigInfo) {
				...
				if(strlen($event_date) == 0 || strlen($det_date) == 0) {
					...
					if(strlen($event_date) == 0) $event_date = $this->min_date($dates);
				...
				$results['verbatimCoordinates'] = $this->getVerbatimCoordinates($rawStr);
				...
				if($cntLongLength > 0) {
					...
					if(array_key_exists('latitude', $latLong)) {
						...
						$results['decimalLatitude'] = round($this->getDigitalLatLong($lat), 3);
					}
					if(array_key_exists('longitude', $latLong)) {
						$long = $latLong['longitude'];
						$results['decimalLongitude'] = round($this->getDigitalLatLong($long), 3);
					}
				}
				$recordedBy = "";
				$recordedById = "";
				...
				if(strlen($recordedBy) == 0 || strlen($recordedById) == 0 || strlen($recordNumber) == 0) {
		...
		$resultsOut = array_merge($politicalConfigInfo, $results);
		//Some cleaning: Some of the values coming out are arrays (e.g. eventdate is array('year'=>####,'month'=>##,day=>##))
		foreach($resultsOut as $k => $v){
			if(is_array($v)){
				$tempArr = $resultsOut[$k];
				unset($resultsOut[$k]);
				$resultsOut = array_merge($resultsOut,$tempArr);
			}
		}
		return $resultsOut;
	}
	...
	private function getDigitalLatLong($latlong) {
		...
		if($degPos !== FALSE) {
			...
			if(strcasecmp($direction, 'W') == 0 || strcasecmp($direction, 'S') == 0) return -1*($degs + $digMins + $digSecs);
	...
	private function getStateFromCounty($c, $ss=null, $country="") {
		if($c) {
			...
			$sql = "SELECT cr.countryName, sp.stateName FROM (lkupstateprovince sp ".
				...
				"ON (sp.stateid = c.stateid) WHERE c.countyName = '".$c."'";
			if(strlen($country) > 0) $sql .= " AND cr.cr.countryName = '".$country."'";
			//echo "\n\nSQL: ".$sql."\n\n";
			if($r2s = $this->conn->query($sql)) {
	...
	private function getCountryFromState($s) {
		if($s) {
			...
			$sql = "SELECT c.countryName FROM lkupstateprovince sp INNER JOIN lkupcountry c ".
				"ON (sp.countryid = c.countryid) WHERE sp.stateName = '".$s."'";
			//echo "\n\nSQL: ".$sql."\n\n";
			if($r2s = $this->conn->query($sql)) {
				$num_rows = $r2s->num_rows;
	...
	private function isStateOrProvince($sp) {
		if($sp) {
			$sql = "SELECT * FROM lkupstateprovince lusp WHERE lusp.stateName = '".$sp."'";
			if($r2s = $this->conn->query($sql)){
	...
	private function isCountryInDatabase($c) {//echo "\nInput to isCountryInDatabase: ".$c."\n";
		if($c) {
			$sql = "SELECT * FROM lkupcountry luc WHERE luc.countryName = '".$c."'";
			if($r2s = $this->conn->query($sql)){
				if($r2 = $r2s->fetch_object()) return true;
			}
		}
	...
	private function processCollectorQueryResult($r2, $name, $string) {//echo "\nInput to processCollectorQueryResult: ".$name."\n";
		$firstName = $r2->firstName;
		$firstPart = trim(substr($string, 0, strpos($string, $name)));
		$fNamePat = "/(?:.*)\\b([a-zA-Z])(?:[.-_]\\s|\\s|[.-_])([a-zA-Z])\\b\\.?/i";//First Initial, Middle Initial
		$middleInitial = $r2->middleInitial;
		if(preg_match($fNamePat, $firstPart, $mats)) {//$i=0;foreach($mats as $mat) echo "\nline 960, mats[".$i++."] = ".$mat."\n";
			$mats1 = $mats[1];
			$mats2 = $mats[2];
			if(STRLEN($firstName) > 0 && strcmp(substr($firstName, 0, 1), $mats1) == 0) {
				if(STRLEN($middleInitial) > 0) {
					if(strcasecmp(substr($middleInitial, 0, 1), $mats2) == 0) {
						return array
						(
							'collectorID' => $r2->recordedByID,
							'familyName' => $name,
							'collectorName' => $r2->firstName." ".$middleInitial." ".$name
						);
					}
				} else {
					return array
					(
						'collectorID' => $r2->recordedByID,
						'familyName' => $name,
						'collectorName' => $r2->firstName." ".$name
					);
				}
			}
		} else {
			$fNamePat = "/(?:.*)\\b([a-zA-Z]{2,}+)\\s([a-zA-Z])\\b\\.?/i";//First Name, Middle Initial
			if(preg_match($fNamePat, $firstPart, $mats)) {//$i=0;foreach($mats as $mat) echo "\nline 984, mats[".$i++."] = ".$mat."\n";
				$mats1 = $mats[1];
				$mats2 = $mats[2];
				if(STRLEN($firstName) > 0 && strcasecmp(substr($firstName, 0, 1), substr($mats1, 0, 1)) == 0) {
					if(STRLEN($middleInitial) > 0) {
						if(strcasecmp(substr($middleInitial, 0, 1), $mats2) == 0) {
							return array
							(
				...
				$fNamePat = "/(?:.*)\\b([a-zA-Z]{2,}+)\\s([a-zA-Z]{2,}+)\\.??/i";//First Name, Middle Name
				if(preg_match($fNamePat, $firstPart, $mats)) {//$i=0;foreach($mats as $mat) echo "\nline 1008, mats[".$i++."] = ".$mat."\n";
					$mats1 = $mats[1];
					$mats2 = $mats[2];
					if(STRLEN($firstName) > 0 && strcmp(substr($firstName, 0, 1), substr($mats1, 0, 1)) == 0) {
						if(STRLEN($middleInitial) > 0) {
							if(strcasecmp(substr($middleInitial, 0, 1), substr($mats2, 0, 1)) == 0) {
								return array
								(
									'collectorID' => $r2->recordedByID,
									'familyName' => $name,
					...
					$fNamePat = "/(?:.*)\\b([a-zA-Z]+)/i";//First Name only
					if(preg_match($fNamePat, $firstPart, $mats)) {//$i=0;foreach($mats as $mat) echo "\nline 1032, mats[".$i++."] = ".$mat."\n";
						$mats1 = $mats[1];
						if(STRLEN($firstName) > 0 && strcmp(substr($firstName, 0, 1), substr($mats1, 0, 1)) == 0) {
							if(STRLEN($middleInitial) > 0) {
								return array
		...
		if(strlen($name) > 2 && !preg_match("/\\b(?:on|var\\.?|strain|subsp\\.?|ssp\\.?|f\\.)\\b/i", $name)) {
			...
			if($pos !== FALSE) {
				...
				if(count($words) > 3 && (strlen($word0) < 4 || strlen($word1) < 4 || strlen($words[2]) < 4)) {
					$name2 = $word0.trim($word1).trim($words[2]);
					$sql = "SELECT * FROM taxa t WHERE t.sciName = '".$name2."'";
					//echo "\nline 1072, sql: ".$sql."\n";
					if($r2s = $this->conn->query($sql)) if($r2s->num_rows > 0) return true;
				}
				if(count($words) < 6) {
					$name2 = str_replace(array("1", "!", "|", "5", "0"), array("l", "l", "l", "S", "O"), $word0." ".trim($word1));
					$sql = "SELECT * FROM taxa t WHERE t.sciName = '".$name2."'";
					//echo "\nline 1078, sql: ".$sql."\n";
					if($r2s = $this->conn->query($sql)) if($r2s->num_rows > 0) return true;
				}
			}
			if(preg_match("/^\\w{3,24}\\s\\w{3,24}\\s\([a-zA-Z01ü&. ]{1,14}\\.?\\s?\)\\s?(?:[A-Z]\\.\\s){0,2}[a-zA-Z01ü&. ]{2,19}\\.?$/", $name)) {
				//echo "\nline 1083, match\n";
				return true;
			}
			if(preg_match("/^[a-zA-Z01]{3,24}\\ssp\\./", $name)) return true;
		}
		return false;
	}
	...
	private function getCollectorFromDatabase($name, $string) {
		if(strlen($name) > 2) {//echo "\nInput to getCollectorFromDatabase, Name: ".$name.", String: ".$string."\n";
			$sql = "SELECT c.recordedByID, c.familyName, c.firstName, c.middleInitial ".
				"FROM omcollectors c ".
				"WHERE c.familyName = '".$name."'";// AND c.recordedByID != 8024";

			if($r2s = $this->conn->query($sql)) {
				if($r2s->num_rows == 1) {
					if($r2 = $r2s->fetch_object()) {
						//echo "\nSQL: ".$sql."\n\n";
						return $this->processCollectorQueryResult($r2, $name, $string);
					}
				} else {
					while($r2 = $r2s->fetch_object()){
						$result = $this->processCollectorQueryResult($r2, $name, $string);
	...
	private function getCounty($c) {
		if($c) {
			if(strlen($c) > 2) {
				$result = array();
				$c = trim($c, " \t\n\r\0\x0B,.:;!()\"\'\\~@#$%^&*_-");
				//echo "\nLine 1201, Input to getCounty: ".$c."\n";
				//return null;
	...
	private function processCountyString($cString, $nextWord = null) {
		...
		//echo "\nLine 1497, Input to processCountyString: ".$cString.", nextWord: ".$nextWord."\n";
		if(strlen($cString) > 0) {
			...
			if($statePos !== FALSE) {
				...
				if($cArray != null) {
					return $this->processCArray($cArray, $cString);
				}
			}
	...
	private function findCounty($c, $state_province="") {//echo "\nLine 1711, Input to findCounty: ".$c."\n";
		$countyPatStr = "/(?:(?(?=(?:.*+)(?:(?:[!|150a-zA-Z]+(?:\\.?\\s?[!|150a-zA-Z]*))".
			"(?:\\.?\\s?[!|150a-zA-Z]*)?)?\\s?\\b(?:C[o0q]un[tf]y|Parish)(?!(?:(?i)\\s(?:Road|Hiway|Hwy|Highway|line)))\\b[,:]?(?:.*)))".
			...
			"(?:(?(?=(?:.*)\\b(?:C[o0q](?:(?:unty)|\\.)|Parish)(?!(?:(?i)\\s(?:Road|Hiway|Hwy|Highway|line)))\\b[,:]?+(?:.*)))".

			"(.*)\\b(?:C[o0q](?:(?:unty)|\\.)|Parish)(?!(?:(?i)\\s(?:Road|Hiway|Hwy|Highway|line)))\\b[,:]?+(.*)|".

			"(?:(?(?=(?:.*)\\b(?:Co[,.]|Go\\.|Parish)(?!(?:(?i)\\s(?:Road|Hiway|Hwy|Highway|line)))\\b:?+(?:.*)))".

			"(.*?)\\b(?:Co[,.]|Go\\.|Parish)(?!(?:(?i)\\s(?:Road|Hiway|Hwy|Highway|line)))[,:]?\\b:?+(.*?)|".

			"(.*?)(?:\\n\\r|\\r\\n|\\n|\\r|\\s)(?:Co[,.]|Go\\.|Parish)(?!(?:(?i)\\s(?:Road|Hiway|Hwy|Highway|line)))[,:]?+(.*))))))/is";
		...
		if(preg_match($countyPatStr, $tempC, $countyMatches)) {
			...
			//echo "\nline 1884, leaving findCounty, firstPart: ".$firstPart."\nsecondPart: ".$secondPart."\ncounty: ".$county."\nstate: ".$state."\n";
	...
	private function getTaxonOfHeaderInfo($str) {
		...
		if(preg_match($statePatStr, $str, $matches)) {
			//$i=0;
			//foreach($matches as $matche) echo "\nmatches[".$i++."] = ".$matche."\n";
			if(preg_match($statePatStr, $matches[1], $matches2)) {
				//$i=0;
				//foreach($matches2 as $matche2) echo "\nmatches2[".$i++."] = ".$matche2."\n";
	...
	private function processTaxonOfHeaderInfo($matches) {
		...
		//echo "\nline 1919, startOfFile: ".$startOfFile."\n\tendOfLine: ".$endOfLine."\n\tendOfFile: ".$endOfFile."\n";
		...
		if($commaPos !== FALSE) {
			...
			if(count($sp) > 0) {
				...
				if($countyMatches != null) {
					...
					$municipality = trim($countyMatches[4]);
				} else {
				...
				if(strlen($country) > 0) {
					...
					if(count($sp) > 0) {
						$state_province = $sp[0];
						if(preg_match("/(.*)\\s(?:Co(?:\\.|unty)|Parish|Dist(?:\\.|rict))\\b/i", $firstPart, $cMatches)) {
							$county = trim($cMatches[1]);
						}
						else $municipality = $firstPart;
						$country = $sp[1];
					} else {
						$sp = $this->getStateOrProvince($firstPart);
						if(count($sp) > 0) {
							$state_province = $sp[0];
							if(preg_match("/(.*)\\s(?:Co(?:\\.|unty)|Parish|Dist(?:\\.|rict))\\b/i", $secondPart, $cMatches)) {
								$county = trim($cMatches[1]);
							}
							else $municipality = $secondPart;
							$country = $sp[1];
			...
			if(count($ps) > 0) {
				...
				if(strcasecmp($state_province, $endOfLine) != 0) {
					if(preg_match("/(.*)\\s(?:Co(?:\\.|unty)|Parish|Dist(?:\\.|rict))/i", $endOfLine, $cMatches)) {
				...
				if(strlen($country) == 0) {
					if(preg_match("/(.*)\\s(?:Co(?:\\.|unty)|Parish|Dist(?:\\.|rict))/i", $endOfLine)) {
						$countyMatches = $this->findCounty($endOfLine, $state_province);
						if($countyMatches != null) {
							if(strlen($state_province) == 0) $state_province = trim($countyMatches[5]);
							if(strlen($location) == 0) $location = trim($countyMatches[0]);
		...
		(!is_numeric($temp) &&
			!preg_match("/\\b(?:lichens?|loc(?:\\.|ality|ation)|loc(?:\\.|ality|ation)|Herbarium|Univers|".
				"County|Parish|Park|Island|Quadrangle|Dist(?:\\.|rict)|U\\.?S\\.?A\\.?|NEW Y\\wRK|".
		...
		if(strlen($county) == 0) {
			...
			if($countyMatches != null) {
				...
				if(strlen($country) > 0 && strlen($state_province) == 0) {
					if(strcasecmp($country, "U.S.A.") == 0 || stripos($country, "United States") !== FALSE) {
						$ps = $this->getStateFromCounty($county);
						if(count($ps) > 0) $state_province = $ps[0];
					} else if(strlen($thirdPart) > 0) {
						$states = $this->getStatesFromCountry($country);
	...
	private function doFarlowLabel($s) {//Farlow labels have the sciName after the institution identifiers followed by substrate, if given
		...
		if($identifier != null) {
			...
			//$id_date = $identifier[1];
			//if(array_key_exists('year', $id_date) $id_date['year'] = $id_date['year'];
	...
	private function isFloraOfAlaskaLabel($s) {//there are 2 kinds of FloraOfAlaska labels: The ones with AKFWS herbarium
		//and those with a Lat/Long label
		$akfwsPat = "/.*FL[O0]\\wA\\s[O0Q]\\w\\sA[1Il!|]A[S5]KA.*/is";
		if(preg_match($akfwsPat, $s)) return true;
		else return false;
	}
	...
	private function processSciName($name) {//echo "\nInput to processSciName: ".$name."\n";
		if($name) {
			$name = trim(preg_replace(array("/[\r\n]/m", "/\\s{2,}/m"), " ", $name));
			$recordNumber = "";
			...
			if($this->isPossibleSciName($name)) return array('scientificName' => $name, 'recordNumber' => $recordNumber);
				...
				if(!$foundSciName && preg_match("/(.*)\\s(var\\.?|ssp\\.?|f\\.|subsp\\.?)\\s(.*)/i", $name, $mats)) {
					...
					if($this->isPossibleSciName($potentialSciName)) {
						...
						$results['recordNumber'] = $recordNumber;
						$temp = trim($mats[3]);
						if(preg_match("/(.*?)(?:\\s(?:Found|Common|Loose))?\\s(on\\s.*)/i", $temp, $mats2)) {
							$substrate = $mats2[2];
						...
						if(strlen($temp) > 3) {
							$results['taxonRank'] = str_replace("ssp", "subsp", strtolower($mats[2]));
							$results['infraspecificEpithet'] = $temp;
						}
					}
				}
				if(!$foundSciName && preg_match("/(.*)(?:\+|(?:\\sfound|\\sassociated)?\\swith\\s)(.*)/i", $name, $mats)) {
					$potentialSciName = trim($mats[1]);
					if($this->isPossibleSciName($potentialSciName)) {
						$foundSciName = true;
						$results['scientificName'] = $potentialSciName;
						$associatedTaxa = trim($mats[2]);
						if($this->isPossibleSciName($associatedTaxa)) $results['associatedTaxa'] = $associatedTaxa;
						else if(preg_match("/(.*)\\s(on\\s.*)/i", $associatedTaxa, $mats2)) {
							$potentialAssTaxa = $mats2[1];
							if($this->isPossibleSciName($potentialAssTaxa)) {
								$results['associatedTaxa'] = $potentialAssTaxa;
								$substrate = $mats2[2];
								if(!preg_match("/\\b(?:HWY|Highway|road)\\b/i", $substrate)) $results['substrate'] = $substrate;
							}
						}
				...
				if(!$foundSciName && preg_match("/(.{4,})(?:\\s(?:Found|Common|Loose))?\\s(on\\s.*)/i", $name, $mats)) {
					$potentialSciName = $mats[1];
					if($this->isPossibleSciName($potentialSciName)) {
						$foundSciName = true;
						$results['scientificName'] = $potentialSciName;
	...
	private function doNewStyleAKFWSLabel($s) {//new style labels have the AKFWS Herbarium followed by ** Flora of Alaska
		...
		if(preg_match($patStr, $s, $mat)) {
			...
			else if(preg_match("/(.*)C[o0][li1!|]{1,2}\\.?\\sDate/i", $municipality, $mat)) $municipality = $mat[1];
		}
		$possibleMonths = "Jan(?:\\.|(?:uary))?|Feb(?:\\.|(?:ruary))?|Mar(?:\\.|(?:ch))|Apr(?:\\.|(?:il))?|May|Jun[.e]?|Jul[.y]?|Aug(?:\\.|(?:ust))?|Sep(?:\\.|(?:t\\.?)|(?:tember))?|Oct(?:\\.|(?:ober))?|Nov(?:\\.|(?:ember))?|Dec(?:\\.|(?:ember))?";
		$identifier = $this->getIdentifier($s, $possibleMonths);
	...
	private function doOldStyleAKFWSLabel($s) {//old style labels have the AKFWS Herbarium followed by Flora of Alaska (no **) on the same or next line
		...
		$elevationArray = $this->getElevation($s);
		if($elevationArray != null && count($elevationArray) > 0) $elevation = $elevationArray[1];
		$patStr = "/(?:M|H|IX|fl|Il)ap\/[QO0G]ua[ad]\\.?(.+)/i";
		$municipality = '';
		if(preg_match($patStr, $s, $mat)) {
			$municipality = $mat[1];
			if(strpos($municipality, "(") !== FALSE) $municipality = substr($municipality, 0, strpos($municipality, "("));
				...
				if(preg_match("/(.*)[0OGD]uad/i", $municipality, $mat)) $municipality = $mat[1];
	...
	private function isLichensOfLabel($s) {
		//$lichensOfPat = "/.*[UL][1Il!|]ch[CE]ns\\s[O0Q]F\\b.*/is";
		$lichensOfPat = "/.*(?:U|L[1Il!|]|[1Il!|])ch[CE]ns\\s[O0Q]F\\b.*/is";
		if(preg_match($lichensOfPat, $s)) return true;
		else {
			$lichensOfPat = "/.*hens\\s[O0Q]F\\b.*/is";
			if(preg_match($lichensOfPat, $s)) return true;
			else return false;
		}
	}
	...
	private function doMerrilLichenesExsiccatiLabel($s) {
	...
	private function isLichensOfFloridaLabel($s) {
		...
		if(preg_match($lfPat, $s, $mat)) return true;
		else return false;
	}
	...
	private function isLichensAndMossesOfYellowstoneLabel($s) {
		...
		if(preg_match($yPat, $s, $mat)) return true;
		else return false;
	}
	...
	private function doMTLichensOfLabel($s) {
		...
		$montanaPat = "/(?:U|L[1Il!|]|[1Il!|])ch[CE]ns\\s[O0Q]F\\sM[O0Q]ntana(.*)/is";
		if(preg_match($montanaPat, $s, $mats)) {
			$s = trim($mats[1]);
			$state_province = "Montana";
		}
		else {
			$pat = "/(?:U|L[1Il!|]|[1Il!|])ch[CE]ns\\s[O0Q]F\\sYell[O0Q]wstone\\sNat[1Il!|][O0Q]na[1Il!|]\\sPark(.*)/is";
			if(preg_match($pat, $s, $mats)) $s = trim($mats[1]);
			else {
				$pat = "/LEW\\s?[1Il!|]S\\s?AND CLARK\\s?C\\s?AV\\s?ERNS\\s?STATE\\sPARK(.*)/s";
				if(preg_match($pat, $s, $mats)) {
					$s = trim($mats[1]);
					$state_province = "Montana";
					$county = "Jefferson";
				} else {
					$pat = "/(?:U|L[1Il!|]|[1Il!|])ch[CE]ns\\s[O0Q]F\\sHEADWATERS\\s?STATE\\sPark(.*)/is";
					if(preg_match($pat, $s, $mats)) {
						$s = trim($mats[1]);
						$state_province = "Montana";
						$county = "Gallatin";
					} else {
						$pat = "/(?:U|L[1Il!|]|[1Il!|])ch[CE]ns\\s[O0Q]F\\s[CG]rand\\s?Tet[O0Q]n\\s?Nati[O0Q]na[1Il!|]\\sPark(.*)/is";
						if(preg_match($pat, $s, $mats)) {
							$s = trim($mats[1]);
							$state_province = "Wyoming";
							$county = "Teton";
						} else {
							$pat = "/(?:U|L[1Il!|]|[1Il!|])ch[CE]ns\\s[O0Q]F\\sWASH[1Il!|]NGTON\\sSTATE(.*)/is";
							if(preg_match($pat, $s, $mats)) {
								$s = trim($mats[1]);
		...
		$elevArr = $this->getElevation($s);
		$temp = $elevArr[0];
		if(strlen($temp) > 0) $elevation = $elevArr[1];
		...
		foreach($lines as $line) {//echo "\nline 6233, line: ".$line."\n";
			...
			if($psn != null) {
				...
				$firstPart = trim(substr($s, strpos($s, $line)+strlen($line)));
				break;
			}
		}
		if(strlen($firstPart) > 0) {
			if(preg_match("/^(On\\s.+?),?\\s(in\\s.+?),\\s(.+)/i", $firstPart, $mats)) {//$i=0;foreach($mats as $mat) echo "\nline 6192, mats[".$i++."] = ".$mat."\n";
				$substrate = $mats[1];
				$habitat = trim($mats[2]);
	...
	private function doBorealiAmericaniLabel($s) {
		...
		$lines = explode("\n", $s);
		foreach($lines as $line) {//echo "\nline 6334, line: ".$line."\n";
			if(preg_match("/.*[BS]\\sB[o0]real[1Il!|]-Amer[1Il!|]can[1Il!|].*/i", $line)) continue;
			if(preg_match("/.*[1Il!|]{3,5}ams and A. H. Seym[o0].*/i", $line)) continue;
			if(preg_match("/.*(?:\\spublish|Second|edition|prepare|\\sdistribut|\\sUnivers|\\sDUKE\\sU|erbarium|Coll[,.]\\s|\\sDecades).*/i", $line)) continue;
			$line = trim($line, " \t\n\r\0\x0B,:;!\"\'\\~@#$%^&*_-");
			if(preg_match("/^.{0,3}?([\]\[OQSZlU|I!0-9&]{1,3}[a-fA-F]?)[.,_]?+\\s(.*)/", $line, $mats) && strlen($line) > 6 && !$this->isMostlyGarbage($line, 0.60)) {
				$exsnumber = $this->replaceMistakenNumbers(trim($mats[1]));
				$scientificName = trim($mats[2]);
				$line = "";
			}
			...
			if(preg_match("/=?\\s?N[,.]?\\s?A[,.]?\\s?[a-zA-Z]{1,2}\\.?\\s?([OQSZlU|I!0-9&]{1,3}[a-fA-F]?)[,.]?/", $line, $mats)) {
				$temp = $this->replaceMistakenNumbers(trim($mats[1]));
				if(is_numeric($temp)) {
					if($temp > 0) $otherCatalogNumbers = "N. A. L. ".$temp;
				} else $otherCatalogNumbers = "N. A. L. ".$temp;
			} else if(preg_match("/=?\\s?.{1,4}[,.]?\\s?A[,.]?\\s?L\\.?\\s?([OQSZlU|I!0-9&]{1,3}[a-fA-F]?)[,.]?/", $line, $mats)) {
				$temp = $this->replaceMistakenNumbers(trim($mats[1]));
				if(is_numeric($temp)) {
					if($temp > 0) $otherCatalogNumbers = "N. A. L. ".$temp;
				} else $otherCatalogNumbers = "N. A. L. ".$temp;
	...
	private function convertSlashedDates($string) {//echo "\nInput to convertSlashedDates: ".$string."\n";
		...
		if($c > 0) {
			...
			if($c == 3) return $year."-".$tokens[0]."-".$tokens[1];
			if($c == 2) return $year."-".$tokens[0];
			return $year;
		}
		return "";
	}
	...
	private function doLichensAndMossesOfYellowstoneLabel($s) {
		...
		foreach($lines as $line) {
			...
			if(preg_match("/^(?:No.+:\\s)?([A-Z]{2,3}\\d{2,6})/", $line, $mats)) $recordNumber = trim($mats[1]);
	...
	private function getPoliticalConfigInfo($str) {
		if($str) {
			if($this->isFarlowLabel($str)) return $this->doFarlowLabel($str);
				...
				$verbatimAttributes = '';
				$habitat = '';
				$municipality = $this->getMunicipality($str);
				$location = "";
				...
				if($matches != null) {
					...
					if(array_key_exists('municipality', $info) && ($municipality == null|| strlen($municipality) == 0)) $municipality = $info['municipality'];
					if(array_key_exists('substrate', $info) && ($substrate == null|| strlen($substrate) == 0)) $substrate = $info['substrate'];
					if(array_key_exists('scientificName', $info) && ($scientificName == null|| strlen($scientificName) == 0)) $scientificName = $info['scientificName'];
				...
				$quadPatStr = "/\\bQu[ao][ad]\\.?(?:\\sM[ao]p)?[.;:]?\\s(.*)/i";
				if(preg_match($quadPatStr, $str, $quadMatches)) {
					$municipality = trim($quadMatches[1]);
					$mLength = strlen($municipality);
					if($mLength > 0) {
						$commaPos = strpos($municipality, ",");
						if($commaPos !== FALSE && $commaPos<$mLength-1) {
							$rest = trim(substr($municipality, $commaPos+1));
							$rLength = strlen($rest);
							$municipality = trim(substr($municipality, 0, $commaPos));
							$eMuns = explode(" ", $municipality);
							$temp = "";
							...
							if(strlen($temp) > 0) {
								$rest = substr($municipality, strpos($municipality, $temp));
								$municipality =
									str_replace
									(
										array("0", "1", "!", "|", "5", "2"),
								...
								$paren1Pos = strpos($municipality, "(");
								if($paren1Pos !== FALSE) {
									$rest = trim(substr($municipality, $paren1Pos+1));
									...
									if($paren2Pos !== FALSE) {
										$municipality =
											...
											(
												array("0", "1", "!", "|", "5", "2"),
												array("O", "l", "l", "l", "S", "Z"),
												trim(substr($municipality, 0, $paren1Pos))
											);
									}
								}
							} else $municipality = "";
							...
							if($paren1Pos !== FALSE) {
								...
								$paren2Pos = strpos($rest, ")");
								if($paren2Pos !== FALSE) {
									$rest = trim(substr($municipality, $paren2Pos+1));
									$rLength = strlen($rest);
									$municipality =
										str_replace
										(
				...
				if(strlen($substrate) > 0) {
					if(strlen($location) > 0) {
						...
						if($pos !== FALSE) {
							if($pos == 0) $location = trim(substr($location, strlen($substrate)));
							else {
								$temp = "On ".$substrate;
				...
				if(strlen($county) > 0 && strlen($state_province) > 0) {
					$pos = stripos($county, $state_province);
					if($pos !== FALSE) $county = trim(substr($county, $pos+strlen($state_province)), " \t\n\r\0\x0B,:;.!\"\'\\~@#$%^&*_-");
				}
				if(preg_match("/(.*)[0OD]at[ec]/i", $municipality, $mat)) $municipality = $mat[1];
				if(strlen($scientificName) == 0) {
					$lines = explode("\n", $str);
					foreach($lines as $line) {
						$psn = $this->processSciName($line);
	...
	private function getIdentifier($str, $possibleMonths) {
		$detPatStr = "/\\b(?:(?:D[ec][trf](?:\\.|[ec]rmin[ec](?:d b[vy]|er[sz]?)))|(?:[Il!|]d[ec]nt[Il!|]f[Il!|][ec]d b[vy]))[;:]?\\s?(.+)(?:(?:\\n|\\r\\n)((?s).+))?/i";
	...
	private function terminateField($f, $regExp) {
		...
		if(!$this->isMostlyGarbage2($temp, 0.50)) return $temp;
		else return "";
	}
	...
	private function getElevation($string) {
		if(strlen($string) > 0) {//echo "\nInput to getElevation: ".$string."\n";
			...
			if(preg_match($elevPatStr, $string, $elevMatches)) {
				...
				$elevation = str_ireplace
				(
					array("^", "O", "Q", "l", "|", "I", "!", "S", "Z", "tt", "rn", "\r\n", "\n", "\r", "&", "ab0ut", "ab0ve", "ab0w", "abow"),
					array("5", "0", "0", "1", "1", "1", "1", "5", "2", "ft", "m", " ", " ", " ", "6", "about", "above", "above", "above"),
					$elevation
				);
				return array($firstPart, $elevation, $nextPart);
			}
			$elevPatStr = "/(?:(?(?=(?:.*?)(?:(?:E[li1|][ce]v(?:ati[o0][nr]|\\.)?|Alt(?:\\.|itude)?)[:;,]?)\\s".
				...
				"(?:\\s?(?:ft\\.?|tt\\.?|m\\.?|rn\\.?|Feet)?\\s?-\\s?(?:[l|I!1]".$possibleNumbers.",".$possibleNumbers."{3}|".
				"[SZl|I!&1-9],".$possibleNumbers."{3}|".
				"[SZl|I!&1-9^]".$possibleNumbers."{1,4}|[1-9]))?\\s{0,2}(?:ft\\.?|tt\\.?|m\\.?|rn\\.?|Feet))\\b[,;]?(.*+))/is";
			...
			$elevPatStr = "/(?:(?(?=(?:.*?[^°])\\s(?:(?:ab[o0]ut\\s|ab[o0](?:ve|w)\\s|ca\\.?\\s|[-~])?".
				...
				"(?:\\s?(?:ft\\.?|tt\\.?|m\\.?|rn\\.?|Feet)?\\s?-\\s?(?:[l|I!1]".$possibleNumbers.",".$possibleNumbers."{3}|".
				"[SZl|I!&1-9^],".$possibleNumbers."{3}|".
				"[SZl|I!&1-9^]".$possibleNumbers."{1,4}|[1-9]))?\\s{0,2}(?:ft\\.?|tt\\.?|m\\.?|rn\\.?|Feet))\\b[,;]?".
				"(?:(?:\\s|\\n|\\r\\n)?(.*+))|".
	...
	private function isText($s) {
		...
		foreach($splitChars as $splitChar) {
			if(is_numeric($splitChar)) return FALSE;
		}
		return TRUE;
	}
	...
	private function extractCollectorInfo($str, $cStr, $lineAfter=null) {
		...
		if(!preg_match("/^Herb.*/i", $cStr)) {//echo "\nSecond Pattern Not Matched: ".$cStr."\n";
			...
			$cfds = array();
			foreach($collWords as $collWord) {//echo "\nline 8001, collWord: ".$collWord."\n";
				$collWord = trim($collWord, "-,.");
				if(strlen($collWord) > 2 && !is_numeric($collWord)) {
					...
					if($cfds) {
						...
						$collectorNum = $this->terminateCollector($collectorNum);
						...
						}//echo "\nline 7985, collectorName: ".$collectorName.", collectorNum: ".$collectorNum."\n";
						if($this->containsNumber($collectorNum)) {
							...
							if(preg_match("/(?:[1!l|I][89][OQ1!Il|ZS&0-9]{2}|[2Z][O0]{2}[OQ1!Il|ZS&0-9])/", $collectorNum)) $collectorNum = "";
						} else $collectorNum = "";
...
//If not found tries to find a collector name from the database in a likely place on many labels
	private function getCollector($str) {//echo "\nInput to getCollector: ".$str."\n";
		if($str) {
			$collPatternStr = "/(?:(?(?=(?:\\bC[o0D](?:[l1|!i]{1,2}|U)[ec]{2}t[o0]rs?)[:,;.]?\\s(?:.+)(?:\\r\\n|\\n|\\r)(?:.+)))".

				"(?:\\bC[o0D](?:[l1|!i]{1,2}|U)[ec]{2}t[o0]rs?)[:,;.]?\\s(.+)(?:\\r\\n|\\n|\\r)(.+)|".

				"(?:(?(?=\\bC[o0D][l1|!i]{1,2}(?:\\.|[ec]{2}t[ec]d)?\\s(?:and|&)\\s(?:det(?:\\.|ermined)?|[l1|!i]dent[l1|!i]f[l1|!i]ed)".
				"(?:\\s\\w[yxv])?[:,;.]?\\s(?!Date)(?:.+)(?:\\r\\n|\\n|\\r)(?:.*)))".
				...
				"(?:(?(?=\\bC[o0D][l1|!i]{1,2},?(?:[ec]{2}t[ec]d\\s(?:(?:and|&)\\sprepared\\s)?".
				"\\w[yxv]|[.o]|[ec]{2}t[o0D]rs?|[ec]{2}ti[o0D]n data|\\s&\\s[NW][o0D])?[:,;.]?".
				"\\s(?!Date)(?:.+)(?:\\r\\n|\\n|\\r)(?:.*)))".
				...
				"(?:(?(?=\\bC[o0D][l1|!i]{2}[ec]{2}t[ec]d\\sb[yxv][:,;.]?\\s(?:.+)(?:\\r\\n|\\n|\\r)(?:.*)))".
			...
			if(preg_match($collPatternStr, $str, $collMatches)) {
				//$i=0;
				//foreach($collMatches as $collMatche) echo "\ncollMatches[".$i++."] = ".$collMatche."\n";
				$possibleMonths = "Jan(?:\\.|(?:uary))|Feb(?:\\.|(?:ruary))|Mar(?:\\.|(?:ch))|Apr(?:\\.|(?:il))?|May|Jun[.e]?|Jul[.y]|Aug(?:\\.|(?:ust))?|Sep(?:\\.|(?:t\\.?)|(?:tember))?|Oct(?:\\.|(?:ober))?|Nov(?:\\.|(?:ember))?|Dec(?:\\.|(?:ember))?";
				$collector = "";
				$collectorNum = "";
				$identifiedBy = "";
				$nextLine = "";
				$mNum = count($collMatches);
				if($mNum > 18) {
					$temp = trim($collMatches[16]);
					$spacePos = strrpos(" ", $temp);
					if($spacePos !== FALSE) $endWord = trim(substr($temp, $spacePos+1));
					else $endWord = $temp;
					if(preg_match("/C[o0D][l1|!i]{2}[ec]{2}t[ec]d/i", $endWord)) {
						$collector = trim($collMatches[17]);
						$nextLine = trim($collMatches[18]);
					} else {
						$cI = $this->getCollector($collMatches[18]);
					...
					$collector = $collMatches[12];
					$nextLine = $collMatches[13];
				} else if($mNum > 11) {
					$collector = $collMatches[11];
					$nextLine = "";
				} else if($mNum > 10) {
					$collector = $collMatches[10];
					$nextLine = "";
				} else if($mNum > 9) {
					$collector = $collMatches[8];
					$nextLine = $collMatches[9];
				} else if($mNum > 7) {
					$collector = $collMatches[6];
					$nextLine = $collMatches[7];
				} else if($mNum > 5) {
					$collector = trim($collMatches[5]);
					$nextLine = "";
					$identifiedBy = $collector;
				} else if($mNum > 4) {
					$collector = $collMatches[3];
					$nextLine = $collMatches[4];
					$identifiedBy = $collector;
				} else if($mNum > 2) {
				...
				if(strlen($collector) > 0) {
					...
					if(preg_match("/(.*)Acc(?:[,.]|ession)\\s(?:[NW][o0Q][.ou]|#)(.*)/i", $collector, $cMats)) {//$i=0;foreach($cMats as $cMat) echo "\n6916, cMats[".$i++."] = ".$cMat."\n";
						...
						if(preg_match("/(.*)Acc(?:[,.]|ession)?\\s(?:[NW][o0Q][.ou]|#)(.*)/i", $nextLine, $cMats)) {//$i=0;foreach($cMats as $cMat) echo "\n6916, cMats[".$i++."] = ".$cMat.".\n";
							$collector .= " ".trim($cMats[1], " \t\n\r\0\x0B.,:;!\"\'\\~@#$%^&*_-");
							$otherCatalogNumbers = $this->terminateCollector(trim($cMats[2]));
							$nextLine = "";
						}
					}//echo "\nline 8430, collector: ".$collector.", collectorNum: ".$collectorNum."\n";
					...
					if(strlen($collectorNum) > 0) {
						$temp = $this->terminateCollector($collectorNum);
						if($this->containsNumber($temp)) $collectorNum = $temp;
						else $collectorNum = "";
					}//echo "\nline 8493, collector: ".$collector.", collectorNum: ".$collectorNum."\n";
...
//else the form is STR
		...
		if($str) {
			...
			if(preg_match($latLongPatStr, $str, $latLongMatches)) {
				...
				if($latitude != null && strlen($latitude) > 0) {
					...
					$latitude .= strtoupper(trim($latLongMatches[5]));
					...
					$longitude .= str_replace("VV", "W", strtoupper(trim($latLongMatches[9])));
					//echo "\nstrlen(latitude) > 0\n\n";
					...
					if($latitude != null && strlen($latitude) > 0) {
						...
						$latitude .= strtoupper(trim($latLongMatches[15]));
						...
						$longitude .= str_replace("VV", "W", strtoupper(trim($latLongMatches[19])));
						//echo "\nstrlen(latitude) > 0\n\n";
						...
						if($latitude != null && strlen($latitude) > 0) {
							...
							$latitude .= strtoupper(trim($latLongMatches[25]));
							...
							$longitude .= str_replace("VV", "W", strtoupper(trim($latLongMatches[29])));
							...
							if($latitude != null && strlen($latitude) > 0) {
								...
								$latitude .= strtoupper(trim($latLongMatches[35]));
								...
								$longitude .= str_replace("VV", "W", strtoupper(trim($latLongMatches[39])));
	...
	private function fixString($str) {
		...
		if($str) {
			...
			//Remove barcode number for string
			if(preg_match('/\d+/',$this->catalogNumber,$m)){
				$str = str_replace($m[0], "", $str);
			}
			$str = preg_replace
			(
				array("/([A-Za-z]{4,14}+)'(of)\\s(\\w+)/i", "/\\s\)M[u][1!|l|]{2}\\.?\\s?Arg[.,]?/i", "/\(Ach\\.?\)\\sM[\\w01]{2,4}\\.?\\sArg\\.?/i", "/M[Uui][A-Za-z][l1!|I]\\.?\\sArg\\.?/", "/M[Uui][il1!| ]{2,5}\\.?\\sArg\\.?/", "/M[ul10|!\']{3,5}\\.?\\sArg\\.?/", "/U-[S5]-A.{0,2}/", "/\\.{3,}/", "/Lichens of(\\w+)/i", "/\\bQu\\wrcus\\b/i", "/\\bVVYOMING\\b/i", "/\) Â.{0,2}ch\\./", "/\(Â.{0,2}ch\\.\)/"),
				array("\${1} \${2} \${3}", ") Müll. Arg.", "(Ach.) Müll. Arg.", "Müll. Arg.", "Müll. Arg.", "Müll. Arg.", "USA", " ", "Lichens of \${1}", "Quercus", "Wyoming", ")Ach.", "(Ach.)"), $str, -1, $count
			);
...
//if it finds a numeric date with a Roman Numeral month it converts it to a normal month in the format DD-Month-YYYY.
	private function convertRomanNumeralDates($str) {//$str = "19.\/11.1975";
		if($str) {
			$possibleNumbers = "[0-9&OQIil!|ozZsS]";
	...
	private function replaceMissingDegreeSignsInLatLongs($str) {
		if($str) {
			$badLatLongPatStr = "/(.*)\\b((?(?=°)(?:[SZl|IO!\\d]{2,3})|(?:[SZl|I!\\d]{2,3})))([0OQ°])((?(?<=°)(?:[SZl|IO!\\d]{2,3})|(?:[SZl|I!\\d]{2,3}))\\s?'\\s?)".
				...
				"\\s?[\"\'])?\\s?((?:E|W|VV))\\b(.*)/is";
			if(preg_match($badLatLongPatStr, $str, $latLongMatchesBad)) {
