...
<head>
	...
	<title><?php echo $defaultTitle; ?> Crowdsourcing Queue</title>
...
<body>
	...
	<div class='navpath'>
		...
		<b>Crowdsourcing Queue</b>
	</div>
	<!-- inner text -->
	<div id="innertext">
		<h1>Crowdsourcing Queue</h1>
		<?php 
		if($collId){
			//Query form
			...
			<?php
			//List occurrences
			$cArr = $csManager->getCollDetails($collId);
			echo '<h2>'.$cArr['name'].'</h2>';
			if($cArr['instr']) echo '<div style="margin-left:10px;">'.$cArr['instr'].'</div>';
			if($cArr['url']) echo '<div style="margin-left:10px;"><a href="'.$cArr['url'].'">'.$cArr['url'].'</a></div>';

			//Get list of occurrences
			$oArr = $csManager->getQueueArr($collId);
			foreach($oArr as $occid => $occArr){
				
				
			}
		}
		else{
			echo "<h1>Select a Collections</h1>\n";
			$collArr = $csManager->getCollArr();
			echo '<ul>';
			foreach($collArr as $cid => $cname){
				echo '<li><a href="crowdsourcequeue.php?collid="'.$cid.'">'.$cname.'</a></li>'."\n";
			}
			echo '</ul>';
		}
		?>
