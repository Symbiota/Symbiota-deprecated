	...
	<body>
		...
		<div id="innertext">
			...
			if($commentArr){
				...
				$pageNumber = (int)$recCnt / ($start + 1);
				$lastPage = (int)($recCnt / $limit) + 1;
				$startPage = ($pageNumber > 4?$pageNumber - 4:1);
				$endPage = ($lastPage > $startPage + 9?$startPage + 9:$lastPage);
				$hrefPrefix = 'commentlist.php?'.$urlVars."&start=";
				$pageBar = '';
				if($startPage > 1){
					$pageBar .= "<div style='float:left;margin:5px;'>Pages: \n";
				...
				$pageBar .= "Page ".$pageNumber.", records ".($start+1)."-".$endNum." of ".$recCnt;
				$paginationStr = "<div><div style='clear:both;'><hr/></div>\n";
				$paginationStr .= $pageBar;
				$paginationStr .= "<div style='clear:both;'><hr/></div></div>";
				echo $paginationStr;
			}
			?>
			<!-- Option box -->
			<fieldset style="float:right;width:250px;margin:10px;">
				...
				<form name="optionform" action="commentlist.php" method="post">
					...
					<div>
						<input name="" type="hidden" value="<?php echo $collid; ?>" />
