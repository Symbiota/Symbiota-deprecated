...
<html>
<head>
	<title><?php echo $defaultTitle; ?> SEINet Flash Card Quiz</title>
...
<body <?php if($editMode) echo "onload=\"toggleById('projeditor');\"";?>>
	...
	if(!$proj){
		echo "<h1>".$defaultTitle." Flash Card Quiz Categories</h1>"; 
		...
		foreach($projectArr as $pid => $projArr){
			?>
			<h1><?php echo $projArr["projname"]." Flash Card Quiz"; ?></h1>
			<h3>Choose one of the plant lists below to start playing SEINet Flash Card Quiz</h3>
